...
def synth_single(text, output_mp3, tts, api_key, loudnorm=True):
    ...
    try:
        ...
        # clone=False（内置音色）时不解析 ref_audio（mimo_tts 也只在 clone=True 时用它）；
        # ref_audio 为 null 时 resolve_ref 会抛 TypeError，故按 clone 开关条件解析。
        clone = tts.get("clone", True)
        ref_audio = str(resolve_ref(tts["ref_audio"])) if (clone and tts.get("ref_audio")) else None
        synthesize(
            text, wav,
            voice=tts["voice"], clone=clone,
            ref_audio=ref_audio,
