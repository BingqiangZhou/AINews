...
version: "1.5.0"
description: 把一篇已有文章（Markdown/纯文本）改写成**单人专业资讯播客**——对标《声动早咖啡》《晚点》《硅谷 101》的**快报 + 今日深读双层结构**：当日多条要闻简讯（结论先行、详略起伏）+ 1 条信息量最充足的深讲（背景/首先/其次/最后/反向风险），客观陈述为主、信息密度高、关键处给一句精炼自然判断。输出脚本 + TTS 音频 + 标题描述。改写导向（书面文章→专业资讯主理人播报口吻），内置 10 维 rubric（清晰度/节奏/保真，不奖励闲聊腔）+ 评价→修正迭代循环，迭代到市场可用（每维≥4、零虚构）。MiMo 内置音色单次合成单音色音频（不拆段，音色最一致），可接喜马拉雅发布（需确认）。**触发场景**：用户提到"文章转单人播客""单人播客""专业资讯播报""资讯简报""新闻播报体""独白播客""文章做播客""把文章变成播客音频""文章转喜马拉雅播客""solo podcast""文字转播客音频"，或想把一篇现成文章/笔记/博客生成单人资讯播客时使用。本 skill 的输入是**文章/文本**（改写导向），不是录音/音频。
metadata:
  platforms: [boker]
---
...
把一篇**已有文章**改写成一段**市场可用的单人专业资讯播客**。心智模型是**一位专业资讯主理人做当日 AI 资讯简报**（对标《声动早咖啡》《晚点》《硅谷 101》资讯速递）——**快报 + 今日深读双层结构**：当日多条要闻简讯（结论先行、详略起伏）+ 1 条信息量最充足的深讲（背景/首先/其次/最后/反向风险），客观陈述为主、信息密度高、关键处给一句精炼**自然判断**。改写导向（书面文章→专业资讯播报口吻），不是转录清洗，也不是朋友闲聊。
...
**内容生成（改写导向，双层结构）**
3. **心智模型**：把书面文章改写成"专业资讯主理人做资讯简报"的播报——**快报 + 今日深读双层结构**（当日多条要闻简讯 + 1 条信息量最充足的深讲），客观陈述为主、信息密度高、关键处给克制自然判断（不用"点评："书面标签）。不是朋友闲聊，不是净化口语转录。深读选题宁缺毋滥——若当日素材都信息量单薄，退回全快报模式（标 degraded）。
...
### 按插图分段（[SECTION:N] / [SECTION:DEEPDIVE] 分节标记）
分段定义由 article-image-studio（illustrate 模式）产出在 `imgs/segments.json`（文章 ## 章节与插图的权威映射，含 illustration_meta）。**ingest 纯清洗，不注入任何分段标记**——conductor 读 segments.json 按 segment 分组快报要点 + 选今日深读选题，body-writer 每快报节段首标 `[SECTION:N]`、深读段标 `[SECTION:DEEPDIVE]`，标记保留进 `播客_脚本.txt`。solo_tts 在 TTS 前用 `extract_sections.py` 剥离标记，同时落盘 `sections.json`（字符偏移）+ `temp/script_clean.txt`。下游 article-to-video 用 Whisper 把字符偏移映射成音频时间轴，实现「按插图分段切场景」（取代脆弱的文本匹配）。**`[SECTION:DEEPDIVE]` 对 article-to-video 零破坏**——正则 `\w+` 自动匹配、保留字符串 index、video 侧只消费偏移、全程当普通场景切分（已验证）。无 `imgs/segments.json`（文章无 ## 章节或未跑插图）时 conductor 退回平铺要点，不标 `[SECTION:N]`。
...
2. `播客_TTS.mp3` 存在、时长 8-12 分钟、单音色一致、专业资讯腔调（客观陈述为主 + 快报/深读双层节奏 + 克制自然判断）。
