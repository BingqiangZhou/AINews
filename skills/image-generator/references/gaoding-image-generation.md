...
13. 等待浏览器下载完成（5s），从 Chrome 下载目录复制最新文件到输出目录：
    ```bash
    sleep 5
    latest=$(ls -t "${AINews_DOWNLOAD_DIR:-$HOME/Downloads}"/*.{png,jpg,jpeg,PNG,JPG,JPEG} 2>/dev/null | head -1)
...
**回退协议**：失败后默认调用 Agnes 路径（`agnes_image.py`）并记录 `fallback_used: true`。**例外：内容审核拒绝不回退**（见上表，改写重试）。编排器（如 ai-news-digest）的图片生成阶段**整体禁用 Agnes 回退**（只用稿定），任何失败走「改写重试 1 次 → 仍失败则跳过/暂停」。
...
| Chrome 下载目录 | `$AINews_DOWNLOAD_DIR` 或 `config.chrome_download_dir`，默认浏览器下载目录 |
