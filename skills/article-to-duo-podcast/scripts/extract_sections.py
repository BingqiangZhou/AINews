...
**双人播客（article-to-duo-podcast）说明**：本脚本只剥 `[SECTION:N]` 标记，
**不剥** `A：`/`B：` 角色标注（角色标注与 section 标记正交：section 标记独占一行、
角色标注在台词行首）。双人 TTS 的 duo_tts.py 调用本脚本剥 section 标记后，
**再单独**剥角色标注（parse_turns），并重算 sections.json 偏移使其基于剥了
角色标注的纯文本（_recompute_section_offsets）——因为字幕对齐用 Whisper 转录的
是纯台词（无角色标注），偏移必须与之一致。

输入：播客_脚本.txt（含 [SECTION:N] 标记，N=整数 body 段或 OPENING/ENDING 片头片尾；
      双人脚本另含 A：/B： 角色标注，本脚本不处理它们）
输出：
  - script_clean.txt（剥离 [SECTION:N] 标记后的文本；双人脚本仍含角色标注，
    duo_tts 会再剥一次产出最终给 Whisper 的纯文本）
