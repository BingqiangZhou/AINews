...
| `article-to-duo-podcast` | 文章 → 双人对话播客音频（双主持 A=苏打/B=冰糖 + 双音色 TTS） | skill 委派（结构化字段） |
...
[Phase 7] 多格式（按 config.media 开关并行委派，默认开封面+播客，关插图+视频）：
   ...
   ├─ 7c 播客：article-to-duo-podcast（fast_fallback）→ _podcast/播客_TTS.mp3（占集号）[tts_podcast_enabled]
...
| 7c 播客 | `tts_podcast_enabled` | ✅ 开 | 资讯对话播客有分发价值；委派说明指示走 fast_fallback（双人简化生成，duo-scriptwriter 单写手） |
...
Phase 6（文章）完成
        ...
        │
        └─▶ 7c 播客（article-to-duo-podcast, fast_fallback → _podcast/播客_TTS.mp3）
...
- **7c 播客**（`tts_podcast_enabled`）：委派 `article-to-duo-podcast`，**在委派说明里指示下游 agent 走 fast_fallback**（覆写其 config 的 `studio.enabled` 默认，走 duo-scriptwriter 单写手）→ `_podcast/播客_TTS.mp3`（**占集号**，从本 skill config.json 读 `boker_next_episode`）。插图关闭时无需等 7b-prepare（无 segments.json 依赖）。
...
│   ├── prompts/*.md
│   └── NN-{type}-{slug}.png
├── _podcast/                            ← Phase 7c（article-to-duo-podcast）
│   ├── 播客_脚本.txt
│   ├── 播客_标题与描述.txt
│   ├── 播客_TTS.mp3
│   └── state.json
