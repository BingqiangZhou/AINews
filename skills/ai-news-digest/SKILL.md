...
- **7c 播客**（`tts_podcast_enabled`）：委派 `article-to-solo-podcast`，**在委派说明里指示下游 agent 走 fast_fallback**（覆写其 config 的 `studio.enabled` 默认，走 solo-scriptwriter 单写手）→ `_podcast/播客_TTS.mp3`（**占集号**，从本 skill config.json 读 `boker_next_episode`）。插图关闭时无需等 7b-prepare（无 segments.json 依赖）。
