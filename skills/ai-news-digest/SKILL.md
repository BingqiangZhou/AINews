...
| `article-image-studio` | 公众号封面图（900x383）+ 文章插图（两步：prepare + render） | skill 委派（结构化字段，cover/illustrate/batch 三模式） |
| `article-to-solo-podcast` | 文章 → 单人专业资讯播客音频（快报+深读双层结构 + 客观陈述 + 自然判断 + 单音色 TTS） | skill 委派（结构化字段） |
| `article-to-video` | 文章+插图+播客 → 横版视频（hyperframes 动态画面 + 字幕） | 顺序 CLI 脚本 |
| `browser-publisher` | 三平台草稿发布（公众号 API / 喜马拉雅浏览器 / 抖音浏览器） | skill 委派 |

## 命令约定

- **`<py>`** = 插件解析的 Python 解释器。解析顺序：环境变量 `AINews_PYTHON` → 本 skill `config.environment.conda_python` → `python`（PATH）。Claude Code 下由 `SessionStart` 钩子从插件 userConfig 自动注入。始终用 `<py>`。
...
6. **集号单一来源**：`ai-news-digest/config.json` 的 `platforms.boker_next_episode`（本 skill config.json 的单一来源）。
...
[Phase 7] 多格式（按 config.media 开关并行委派，默认开封面+播客，关插图+视频）：
   ├─ 7a 封面：article-image-studio（cover 模式）→ 公众号_封面.png      [cover_enabled]
   ├─ 7b 插图：article-image-studio（illustrate 模式，prepare → render，回写文章）[illustrations_enabled, 默认关]
...
| 7c 播客 | `tts_podcast_enabled` | ✅ 开 | 专业资讯播客有分发价值；委派说明指示走 fast_fallback（单人专业资讯生成，solo-scriptwriter 单写手） |
| 7d 视频 | `video_enabled` | ❌ 关 | hyperframes（HTML 渲染）生成动态画面；依赖插图做背景 |
...
Phase 6（文章）完成
        │
        ├─▶ 7a 封面（article-image-studio cover 模式 → 公众号_封面.png）
        │
        └─▶ 7c 播客（article-to-solo-podcast, fast_fallback 专业资讯播报 → _podcast/播客_TTS.mp3）
...
- **7a 封面**：委派 `article-image-studio`（cover 模式）→ `公众号_封面.png`（900x383）
- **7b-prepare 插图**（仅 `illustrations_enabled`）：委派 `article-image-studio`（illustrate 模式，prompt_only）→ `imgs/segments.json`
...
- **7d 视频**（仅 `video_enabled`，7b-render + 7c 完成后）：顺序调 `article-to-video` 的 4 个 CLI 脚本（hyperframes 新流程）→ `_video/公众号_视频.mp4`

### Phase 8 — 归档（status: `archive`）

内联：
- 校验所有产物存在（文章/摘要/封面/插图/播客/视频）
- 图片压缩（复用 `<scripts>/compress_images.py`）
- reconcile 一致性检查（复用 `<scripts>/reconcile_media.py`）
...
- **喜马拉雅**（浏览器）：上传播客，发布后递增集号（`<scripts>/bump_episode.py`）
- **抖音**（浏览器，可定时）：上传视频
...
├── 公众号_摘要.txt                       ← Phase 6
├── 公众号_封面.png                       ← Phase 7a（article-image-studio cover 模式）
├── _research/
│   └── 事实素材与来源.md                 ← Phase 5（build_research_md.py，article-studio 的权威源）
├── imgs/                                ← Phase 7b（article-image-studio illustrate 模式）
│   ├── outline.md
...
- **`ai-news-digest/config.json`（只读复用）**：`brand`、`cover`、`image`、`tts`、`platforms.boker_next_episode`（集号单一来源）。
