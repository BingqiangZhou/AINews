...
| `storage_root` | string | 输出根目录（`articles/` 的父目录）。默认空串 = 项目根 |
...
| 字段 | 类型 | 默认 | 说明 |
|------|------|------|------|
| `conda_python` | string | `python` | Python 解释器（`<py>`；`AINews_PYTHON` 环境变量优先，其次此字段，最后 PATH 上的 `python`） |
| `ffmpeg` | string | `ffmpeg` | FFmpeg 可执行文件（`AINews_FFMPEG` 环境变量优先） |
| `font` | string | _(空，自动检测)_ | 字体文件（视频字幕用；`AINews_FONT` 环境变量优先，空=按平台默认：Windows `msyh.ttc`） |
