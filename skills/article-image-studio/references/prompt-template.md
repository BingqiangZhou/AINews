...
Step 4「Prompt 构建」的统一骨架（**cover/custom 模式用**）。取代"LLM 自由发挥 20-40 词"的不确定性，用固定槽位保证 prompt 质量、一致性与可复现性。

> **三维法下的角色**：本文件的 6 槽位结构继续作为 cover/custom 的 prompt 骨架。**Style words 槽位的视觉描述词来源**——直接按 outline 的 `style` 字段（三维法的 Style 维度）查 `styles/<style>.md`。Mood 槽位默认 balanced（不暴露为维度）。
>
> **illustrate 模式**走 [prompt-construction.md](prompt-construction.md) 的 Type-Specific 模板（6 个 Type 骨架 + Type×Style×Palette 组合范例），不用本文件的 6 槽位。
...
prompt 正文**只用描述性颜色名**（如 `warm orange` / `cream background` / `暖米白`），**禁止任何 hex 色值**（AI 图像模型会把 `#ED8936` 当成需要渲染进图的文字）。hex 仅写入 prompt 文件的 YAML frontmatter 作为元数据。

完整语义约束见 [palette-colors.md](palette-colors.md)（权威源）。
...
> **图内文字契约**：后端（Gaoding 万相2.7）只消费 prompt 正文，不读 YAML frontmatter。因此凡是需要画进图的具体中文（标题、标签、数据），**必须**写进 prompt 正文，不能只放在 frontmatter。frontmatter 的 `title_text` / `labels` 字段只供下游 `build_segments.py` 提取到 segments.json 用，对生图后端无效。
...
23 个 style 文件（`references/styles/<name>.md`）是**三维法的 Style 维度定义**（运行时维度）。它们的 Visual Elements / Style Rules 段包含宝贵的视觉语言细节（线条质感、版式、装饰元素），必须加载到 prompt 的 Style words 槽位。

**何时加载 style 文件**：cover/custom 模式按入参或 outline 的 `style` 字段（23 值之一）直接查 `styles/<style>.md`；illustrate 模式按 outline 文章级 `style` 字段加载。**不再是"可选查阅"**——Style 是三维法的核心维度，加载 style 文档是拼 prompt 的必经步骤。

> Style → Rendering 派生映射（23 行）见 [style-rendering-mapping.md](style-rendering-mapping.md)，本文件不重复。每个 style 的"典型视觉语言补充词"在对应的 `styles/<name>.md` 里。

**使用规则**：
1. **必读**：拼 prompt 前加载当前 style 的 `styles/<style>.md`，取 Visual Elements / Style Rules 段（部分含 Layout Guidelines）。
2. **只取视觉描述词**（如"马卡龙色块"、"黑墨干性笔触"），**不照搬整个 style 定义**——避免 prompt 过长。
3. style 文件的 Color Palette 段**不读**（全局 palette=warm，由 [palette-colors.md](palette-colors.md) 的 warm 段提供配色）。
4. frontmatter 记录 `style` 字段（三维法的 Style 维度），便于追溯。

**示例**（style=sketch-notes）：
- 不加载：Style words = `hand-drawn sketch, varied line weight, visible stroke texture`（仅 rendering 的通用 cues，缺失风格特征）
- 加载后：Style words = `hand-drawn sketch notes on warm cream paper, varied line weight, soft macaron color blocks (blue/mint/lavender/peach), rounded boxes with small doodles`（完整风格特征）
