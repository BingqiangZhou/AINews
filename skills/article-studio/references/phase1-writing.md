...
禁用短语的**单一权威源**是 `audio-to-social/references/brand-config.md` 的 `## 禁用 AI 腔短语` 段（`validate_content_quality.py` 运行时解析，跨 skill 共享，不在本文件重复罗列以免漂移）。常见的套路包括：序号过渡词（首先/其次/综上…）、套话连接词（值得一提的是/众所周知/毋庸置疑…）、宏大定语（在当今…的背景下 / 随着…的发展）。
