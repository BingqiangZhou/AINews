...
description: >
  ...
  **transcript 模式**：被 audio-to-social 等编排器调用时，输入可是一段录音转录文本（`source_mode: transcript`），
...
| `transcript` | `source_file`（转录文本）+ `article_type` | **转录文本**（零联网、零外部事实） | 被 `audio-to-social` 等编排器调用，输入是录音转录 |
...
- **2a 机器预检**（复用 `audio-to-social/scripts/validate_content_quality.py --platform gongzhonghao`）：标题/摘要/正文字数、小节数、AI 腔。error 必须清零才进 2b。
...
- 备份旧稿（复用 `audio-to-social/scripts/backup_file.py`）→ 覆盖 → 重跑 2a+2b（双主编并行）出 round-(N+1)
...
`<py>` = 插件解析的 Python（`AINews_PYTHON` → `config.environment.conda_python` → `python`）；`<a2s_scripts>` = `{PROJECT_ROOT}/skills/audio-to-social/scripts`。

## 关联 Skills

- **article-to-solo-podcast**：本 skill 的"评分+修正循环"机制参照它的 Phase 3（market_ready + hard_block + scorecard）。
- **audio-to-social**：只读复用其 `validate_content_quality.py` / `backup_file.py` / `brand-config.md`；纯编排版 audio-to-social 在 transcript 模式下委派本 skill 产出文章。
