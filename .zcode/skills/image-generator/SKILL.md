...
- **audio-to-social**（`image-batch-generator`）：直接引用本 skill 的 `references/gaoding-image-generation.md`（Gaoding）或 `references/jimeng-image-generation.md`（即梦）作为操作手册（不经过 skill 调用层，自行操作浏览器）
...
```bash
{config.conda_python} .zcode/skills/image-generator/scripts/agnes_image.py \
...
```bash
{config.conda_python} .zcode/skills/image-generator/scripts/pollinations_image.py \
