...
路径和参数在 `config.json` 中定义（首次运行引导式生成）。下文 `<py>` 代表 `config.environment.conda_python`，`<scripts>` 代表 `.zcode/skills/audio-to-social/scripts`。
...
```
其中 `<a2v_scripts>` = `.zcode/skills/article-to-video/scripts`。产 `<article_dir>/_video/公众号_视频.mp4`。
