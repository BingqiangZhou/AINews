...
- **audio-to-social**：Phase 6 封面生成委托本 skill
...
### Convenience Presets（映射 audio-to-social visual_preset）

支持 audio-to-social 的 4 个 `visual_preset`（`knowledge-card` / `cozy-story` / `bold-warning` / `minimal-opinion`），自动展开为六维策略（含插图 profile 的 Text/Font 覆盖、备选色板、内容信号判定）。→ 完整映射见 [visual-preset-mapping.md](references/visual-preset-mapping.md)。
...
| 依赖 | 用途 | 说明 |
|------|------|------|
| **image-generator skill** | 实际生图（Gaoding/Jimeng/Agnes）| 必须已安装在本仓库 `.zcode/skills/image-generator/` |
...
5. **回退受 config 控制**：Gaoding 失败时，仅当 `cover.fallback_to_agnes == true`（默认 `false`）才回退 Agnes；否则报告失败由调用方决定。与 `audio-to-social` 的"默认不回退"策略一致
...
13. **后端失败显式上报，不静默降级**：`image-generator` 返回 `success: false` 时，本 skill 必须把失败结果 + `fallback_reason` 原样上报调用方，不静默重试、不静默换后端（除非 `cover.fallback_to_agnes: true`）。与 audio-to-social 的"默认不回退，风格一致优先"策略一致。
...
当被 audio-to-social 或 article-illustrator 调用时：
...
- 被编排调用（audio-to-social 等 5 个上层 skill）：零交互，结构化输入 → 结构化输出
