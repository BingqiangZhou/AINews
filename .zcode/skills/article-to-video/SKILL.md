...
`<py>` = `config.environment.conda_python`（`D:\Development\miniconda3\python.exe`）；`<scripts>` = 本 skill 的 `scripts/`。ffmpeg/ffprobe 在 PATH。Whisper 需 CUDA GPU（推荐，Faster Whisper 加速）；CPU 可用但慢。
