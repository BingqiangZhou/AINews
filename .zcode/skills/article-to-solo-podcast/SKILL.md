...
version: "1.4.2"
description: 把一篇已有文章（Markdown/纯文本）改写成单人独白播客——脚本 + TTS 音频 + 标题描述。改写导向（书面文章→主播口吻），内置 10 维 rubric + 评价→修正迭代循环，迭代到市场可用（每维≥4、零虚构）。MiMo voiceclone 单次合成单音色音频（不拆段，音色最一致），可接喜马拉雅发布（需确认）。**触发场景**：用户提到"文章转单人播客""单人播客""独白播客""文章做播客""把文章变成播客音频""文章转喜马拉雅播客""solo podcast""文字转播客音频"，或想把一篇现成文章/笔记/博客生成单人播客时使用。注意与 audio-to-social 区分：那个是录音/音频输入，本 skill 是文章/文本输入、单人独白。
metadata:
  platforms: [boker]
---
...
把一篇**已有文章**改写成一段**市场可用的单人独白播客**。改写导向，不是转录清洗（那是 `audio-to-social` 的 `boker-optimizer` 的活）。
...
`<py>` = `config.environment.conda_python`（`D:\Development\miniconda3\python.exe`）；`<scripts>` = 本 skill 的 `scripts/`。需 `MIMO_API_KEY`。ffmpeg/ffprobe 在 PATH。
