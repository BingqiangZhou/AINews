...
6. **orality-polish**（读 draft_v2）→ `temp/draft_v3.txt`。
...
8. **meta-writer**（读终稿 + source）→ `播客_标题与描述.txt`（标题/简介/标签，craft §9）。**studio 唯一产出 meta 的环节**。Phase 2 完成。
...
`market_ready==false` 时，把 factcheck + judge 合并的 `fix_directives` 路由到 **punch-up + orality-polish**（按 directive 性质分：趣味→punch_up，口语/套路/事实→orality，事实失真优先 orality 删改）→ 产出新 draft → 重跑 host-listen + judge。≤ `config.evaluation.max_fix_rounds`（默认 3）轮。

## --fast 回退
`config.studio.enabled=false` 或 `studio.fast_fallback=true` → Phase 2 退回单 `solo-scriptwriter`（旧路径，仅作快稿/兜底，默认关）。
