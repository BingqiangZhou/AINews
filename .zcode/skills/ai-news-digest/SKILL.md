...
[Phase 6] 文章：委派 article-studio（news 类型 + transcript 模式 + AI 小周人设）→ 公众号_文章.md
   ▼
[Phase 7] 多格式（并行，照搬 audio-to-social 时序）：
   ├─ 7a 封面：article-cover-image-generator → 公众号_封面.png
   ├─ 7b 插图：article-illustrator（prepare → render，回写文章）
   ├─ 7c 播客：article-to-solo-podcast → _podcast/播客_TTS.mp3（占集号）
   └─ 7d 视频：article-to-video（7b 回写 + 7c 音频完成后）→ _video/公众号_视频.mp4
   ▼
...
```
normalize → fetch → prefilter → score → digest → research → article → media → podcast → video → archive → (publish)
```
...
> 建议在进入 Phase 5 前，向用户展示榜单 + 确认是否继续（重资产生成前留人工 gate）。
...
文章完成后，按 [references/delegation-contracts.md](references/delegation-contracts.md) 的并行时序委派：
...
- **7b-prepare 插图**：委派 `article-illustrator`（prompt_only）→ `imgs/segments.json`
- **7c 播客**（7b-prepare 完成后）：委派 `article-to-solo-podcast` → `_podcast/播客_TTS.mp3`（**占集号**，从 audio-to-social config 读 `boker_next_episode`）
- **7b-render 插图**（与 7c 并行）：生图 + 回写文章
- **7d 视频**（7b-render + 7c 完成后）：顺序调 `article-to-video` 的 5 个 CLI 脚本 → `_video/公众号_视频.mp4`
...
- **Phase 4 榜单生成后**：建议向用户展示榜单 + 确认是否继续进 Phase 5+（重资产生成前留人工 gate）。
- **Phase 9 发布前必须截图确认**（公众号草稿箱 / 喜马拉雅上传页 / 抖音发布页）。
